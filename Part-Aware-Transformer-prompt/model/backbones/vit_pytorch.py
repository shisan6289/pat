...
def generate_2d_mask(H=16, W=8, left=0, top=0, width=8, height=8, part=-1, cls_label=True, device='cuda'):
    ...
    mask_ = torch.zeros([len(mask) + 6], device=device)
    # mask_[4:] = mask
    mask_[6:] = mask
    mask_[part] = 1
...
class part_Attention_ViT(nn.Module):
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        # self.num_patches = num_patches
        self.num_patches = num_patches + 2
        self.num_heads = num_heads

        """add"""
        self.scene_prompt_token = nn.Parameter(torch.zeros(5, 2, self.embed_dim))
        trunc_normal_(self.scene_prompt_token, std=.02)
        self.aux_prompt_token = nn.Parameter(torch.zeros(1, 0, self.embed_dim))
    ...
    def forward_features(self, x):
        ...
        """add"""
        self.scene_prompt_type =1
        self.aux_prompt_nums = 0
        self.scene_prompt_nums = 2
        dataset_label = torch.zeros(128, dtype=torch.int64).to("cuda")
        ...
        if self.scene_prompt_type == 1 and (self.aux_prompt_nums > 0 or self.scene_prompt_nums > 0):
            ...
            for i in range(x.size()[0]):
                if isinstance(dataset_label, int):  # for testing
                    """
                    dataset_label = torch.zeros(128, dtype=torch.int64).to("cuda") 创建了一个张量，包含 128 个元素，所有元素都初始化为 0，数据类型为 int64。
                    尽管这个张量的元素是整数值，但它的类型是张量（torch.Tensor），而不是单个整数。因此，dataset_label 不是一个整数类型，而是一个整型张量。
                   在使用 isinstance(dataset_label, int) 检查时，它将返回 False，因为 dataset_label 是一个张量，而不是单个整数。
                    """
                    if dataset_label != -1:
