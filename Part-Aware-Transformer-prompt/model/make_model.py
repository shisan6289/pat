...
class build_vit(nn.Module):
    ...
    def forward(self, x):
        ...
        layerwise_cls_tokens = [t[:, 0] for t in layerwise_tokens]  # cls token for global feature
        ...
        # 如果是测试阶段，则根据self.neck_feat的值决定返回全局特征feat还是最后一个序列的特征layerwise_cls_tokens[-1]。
        else:
